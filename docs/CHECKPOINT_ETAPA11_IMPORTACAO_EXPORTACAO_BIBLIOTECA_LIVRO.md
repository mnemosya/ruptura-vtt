...
## 0-11. Auditoria independente de reconfirmação (rodada seguinte, sem alterar código)

Rodada dedicada a **re-verificar de forma independente** as duas lacunas que a rodada `## 0-10` já afirmava ter fechado — sem confiar no texto de `## 0-10` como prova, repetindo a auditoria documental e o gesto de browser do zero.

**Escopo confirmado (releitura completa, não presumida)**: PRD `§2.1.9` releído por completo — confirma que existem DUAS operações de drag distintas no documento: (1) drag operacional Livro(leitura)→ficha, criando instância de personagem, explicitamente listado como parte da "primeira entrega da Biblioteca do Livro" (uma feature de leitura de livro + ficha de personagem que **não existe em nenhuma parte deste código** — nenhum sumário navegável, nenhum renderizador de capítulo para leitura, nenhum drop-target na ficha); (2) o aditivo, seção "ETAPA 11", pede "vínculos com capítulos" + "drag de conteúdo estruturado, conforme o PRD", com critério de aceite "entidade pode apontar para o capítulo" — este é o drag editorial Biblioteca→documento-de-capítulo, o que foi implementado. Confirmado por leitura direta (não por citação do checkpoint anterior) que a interpretação (2) é a sustentada pelo texto: o aditivo nunca pede navegação por sumário nem drop na ficha: pede um vínculo estruturado apontando para o capítulo, que é exatamente o que `blocos: BlocoCapitulo[]` representa.

**Questão 1 (painel embutido é a Biblioteca?) — reverificada por leitura de código, não por citação**: lido `CamposCapituloSection.tsx`, `bookDragServerActions.ts::buscarConteudoParaVinculoCapitulo` e `src/app/admin/biblioteca/page.tsx` diretamente. Confirmado: o painel chama a mesma `listContentDocumentsForAdmin` que a página real `/admin/biblioteca` usa, com o mesmo `getContentAdminStatus`/RLS, sempre `status` padrão (`published`), nunca expõe rascunho/arquivado, e devolve deliberadamente só `{contentType, slug, nome}` (nunca o payload completo) — nenhuma lógica de busca paralela ou divergente. Interpretação (C) "Biblioteca como workspace editorial" confirmada independentemente.

**Questão 2 (gesto nativo real) — reexecutada do zero nesta rodada, browser real, nenhuma chamada direta a handler de domínio**:
- Servidor local diagnosticado (`lsof`/porta 3000 livre), iniciado via a ferramenta de preview (mesmo `npm run dev`). Um `404` genuíno de Turbopack em `/admin/biblioteca/rascunhos/novo` logo após o primeiro start foi diagnosticado como cache obsoleto (`rm -rf .next` + restart resolveu, confirmado no log do servidor) — não é um bug da aplicação.
- Fixture: 1 usuário admin temporário via `auth.admin.createUser` + `admin_users` (service role, script descartável, credenciais nunca impressas em log permanente), reaproveitando o item publicado real `item:faca`.
- Gesto real via `javascript_tool`, despachando `DragEvent`s reais (`dragstart`→`dragenter`→`dragover`→`dragleave`→(reentrada)→`drop`→`dragend`) com um `DataTransfer` real, no elemento `draggable` de verdade — nunca chamando `onDragStart`/`onDrop` diretamente. Confirmado, com evidência lida ao vivo do DOM/CSS computado, não copiada de nenhum registro anterior:
  - `dragstart`: `dispatched:true`, `dt.types` continha `application/x-ruptura-book-entity`, `dt.getData(...)` = `{"tipo":"book_entity","contentType":"item","slug":"faca","nome":"Faca"}`.
  - `dragover`: `preventDefault()` confirmado (`dispatchEvent` retornou `false`); feedback visual real lido via `getComputedStyle` — borda mudou de `rgb(52,52,62)`/fundo transparente para `rgb(90,160,106)`/`rgb(21,32,24)` (exatamente a cor codificada no componente para `dragOver=true`).
  - `dragleave`: destaque revertido para `rgb(52,52,62)`/transparente — nenhum destaque preso.
  - `drop`: `preventDefault()` confirmado; "Blocos do capítulo" foi de (0) para (1) com "Entidade — item:faca".
  - `dragend`: despachado sem erro.
  - Salvo (versão incrementada) → navegação real para a mesma URL do rascunho (não SPA) → bloco preservado — persistência confirmada por reload real, não estado de memória.
- Casos inválidos, todos via o mesmo `onDrop` real (nunca a função de domínio chamada diretamente): **duplicidade** (segundo drop de `item:faca` aceito no client, 2 blocos → salvar rejeitado no servidor: "Bloco 2: referência duplicada a item:faca..."); **autorreferência** (capítulo publicado, criado rascunho de edição, o próprio capítulo encontrado na busca por já estar publicado, arrastado para si mesmo → aceito no client → rejeitado no servidor: "Bloco 2: um capítulo não pode referenciar a si mesmo."); **item inexistente** (payload forjado com `slug` que o picker real nunca devolveria → aceito no client → rejeitado no servidor: "Bloco 2: referência inexistente na Biblioteca..."); **payload JSON malformado** (`{not valid json`) → ignorado silenciosamente, nenhum bloco adicionado, nenhuma exceção; **MIME ausente** (só `text/plain`) → ignorado silenciosamente; **drop fora da área válida** (despachado num `<header>` sem handler) → nenhum efeito. Todos os 6 casos convergem para a mesma validação de domínio usada pelo botão "+ adicionar", nenhum erro não tratado, nenhuma tela quebrada.
- Console (`read_console_messages`) e rede (`read_network_requests`) verificados ao final de toda a sequência: nenhum erro, nenhuma resposta não-200 além dos 200 esperados.

**Verificações puras re-executadas nesta rodada (não apenas citadas)**: `scripts/dev/validate-editorial-drag.mjs` recompilado e reexecutado — **6/6**; `scripts/dev/validate-import-export-book.mjs` recompilado e reexecutado — **19/19**; `npx tsc --noEmit` — sem erros; `npm run build` — sucesso (servidor de preview parado antes; `next-env.d.ts` revertido após um toque automático do build).

**Achado observado, não uma regressão**: a opção do dropdown "Tipo de conteúdo" na tela de criação mostra "Capítulo (Livro) (adapter dedicado)", enquanto a página de detalhe do conteúdo publicado mostra "legado: Somente leitura (adapter genérico)". São dois textos de UI diferentes (rótulo de tipo vs. diagnóstico por documento) — o segundo é o mesmo achado já documentado em `## 0-9.11` (nenhum adapter canônico dedicado existe para capítulo, usa o fallback genérico da Etapa 1) e continua correto; o primeiro rótulo do dropdown está descrevendo o registro em `contentTypeRegistry.ts` (que tem uma entrada dedicada para "capitulo"), não o adapter de diagnóstico — não é uma contradição, é a mesma limitação já registrada, vista por dois textos de UI distintos. Não corrigido (fora de escopo, não bloqueia o drag).

**Fixtures**: capítulo de teste (draft + publicado + changelog + editor_metadata + book_links) e usuário admin de fixture removidos via SQL/API direta contra o Supabase real ao final — confirmado por `select count(*)` = 0.

**Conclusão desta rodada**: as duas lacunas continuam fechadas, agora confirmadas por uma segunda auditoria totalmente independente (releitura de fontes do zero, leitura de código sem citar o checkpoint, gesto de browser reexecutado do zero, harnesses recompilados e reexecutados). Nenhum bug novo encontrado. Nenhuma linha de código de produção alterada. Nenhum commit dos anteriores (`0a4ab99`/`1b13e27`/`1f9818f`/`b9c68bc`) foi reescrito.

**Status geral da Etapa 11, reconfirmado: Etapa 11 concluída — integração editorial de drag aprovada.**
