...
**Status geral da Etapa 12: Implementação concluída — aceite de browser pendente.**

Esta é a validação integrada contra um Supabase REAL, feita depois da correção 7. Aplicou as 8 migrations pendentes (0025 a 0032, nenhuma delas jamais aplicada antes neste projeto — `list_migrations` só tinha registro até ~0024), rodou a matriz completa de RLS/RPC com identidades reais (owner, Player A, Player B, invited, removed, outsider, anon) via `SET ROLE`/`request.jwt.claim.sub`, reproduziu os dois ataques críticos pedidos (ambos bloqueados), e — durante essa validação — encontrou e corrigiu DUAS falhas concretas adicionais que nenhuma auditoria estática anterior tinha pego (migrations `0033` e `0034`, ver `## 0-8`). Depois dessas duas correções, toda a matriz voltou a passar. A única coisa que falta é o aceite de browser, bloqueado por limitação objetiva do ambiente desta sessão (a aba do Browser pane ficou sem resposta — `navigate` excedeu 300s sem carregar a página).
...
- **Browser check** — não executado. `navigate` no Browser pane excedeu 300s sem carregar a página (aba sem resposta); limitação objetiva do ambiente desta sessão, não um bloqueio de código (ver `## 0-8.10`). Nenhum resultado de browser foi inventado.
...
- **Concorrência SIMULTÂNEA real** (duas requisições no mesmíssimo instante) — não reproduzível por esta interface (chamadas sequenciais via MCP). O mecanismo de proteção (`for update`, índice único parcial, invalidação de sessão na reentrada) foi confirmado; o resultado de tentativas sequenciais foi determinístico e correto — mas isso não é o mesmo que provar uma corrida real de threads. Ver `## 0-8.7`.

## Limitações reais (gaps honestos que permanecem)

- **Concorrência simultânea real não comprovada** (só o mecanismo e o resultado sequencial) — ver `## 0-8.7`.
- **Browser não executado** — única pendência para "Etapa 12 concluída" de verdade; limitação de ambiente, não de código.
- **Roster do `/join` não tratado** (`## 0-7.7`) — `JoinClient` continua recebendo `perfisIniciais` limitado a 0-1 linha para jogador (efeito da correção 6); não restaurado nem substituído por RPC de roster nesta validação, por não haver intenção de produto inequívoca documentada.
