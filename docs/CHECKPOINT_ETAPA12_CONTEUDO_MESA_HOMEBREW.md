...
- **Consequência aceita da migração de client (redação precisa, ver `## 0-6.8`)**: os scripts `scripts/test-character-storage.ts`, `scripts/test-campaign-end-scene.ts`, `scripts/test-campaign-end-round.ts` rodam fora de qualquer contexto de request HTTP, então `getScopedTableClient()` nunca consegue anexar uma sessão real neles (não há cookie para ler) — ficaram **estruturalmente incompatíveis** com `campaign_profiles`/`characters`, não apenas "sem login no momento"; precisariam de reescrita (ex.: client de setup com service role) para voltar a funcionar, fora do escopo desta correção. Já as rotas `/dev/character-sheet`, `/dev/table`, `/dev/join/[campaignId]` continuam alcançáveis sem login (nenhuma delas exige autenticação para carregar a página) — mas, sem uma sessão autenticada, as leituras dessas 6 funções voltam listas vazias e as escritas falham por RLS; com uma sessão real (visitante logado via `/login`), voltam a funcionar plenamente, herdando a mesma autorização de produto do dashboard do narrador. Nenhuma delas fazia parte da verificação executável automatizada; a consequência é aceita explicitamente — "função de desenvolvimento não justifica policy aberta em produção".
