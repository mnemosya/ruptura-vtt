...
**Status geral da Etapa 12: Etapa 12 concluída — validação integrada aprovada.**

Esta é a segunda rodada de validação integrada contra um Supabase REAL, feita depois da correção 7 e da primeira rodada (`## 0-8`). A primeira rodada tinha aplicado as 8 migrations pendentes (0025 a 0032), rodado a matriz completa de RLS/RPC com identidades reais, reproduzido os dois ataques críticos pedidos (ambos bloqueados), e corrigido duas falhas concretas adicionais (migrations `0033`/`0034`) — mas só tinha comprovado concorrência SEQUENCIAL (não simultânea) e não tinha conseguido executar o aceite de browser (Browser pane sem resposta, sem diagnóstico de processo/porta/HTTP). Esta segunda rodada (`## 0-9`) fechou as duas lacunas: comprovou concorrência SIMULTÂNEA real (clientes independentes, sessões reais, `Promise.allSettled`, 7 cenários aprovados), diagnosticou e corrigiu o servidor local, e executou o aceite de browser completo (narrador, Player A, Player B, membro removido, roster) — encontrando e corrigindo, no processo, um bug real de `revalidatePath` chamado durante render que quebrava o aceite de convite.

Antes desta validação, o checkpoint registrava: esta é a SÉTIMA correção, e é ela — não a correção 6 — que efetivamente permitiu o status "Implementação parcial — validação transacional e de RLS em Supabase pendente" antes de ir para Supabase real. Uma auditoria estática posterior à correção 6 encontrou DUAS falhas críticas que a correção 6 não tinha fechado, e que exigiram reabrir o status para **"Implementação parcial — correções críticas de autorização de sessões e vínculo de personagem pendentes."** por uma sessão inteira antes da correção 7 existir:
...
- **Correção 7** (migration `0032`): fecha as duas falhas críticas relatadas pela auditoria pós-correção-6 (`profile_sessions` com CRUD aberto a `anon`/`authenticated`; `active_character_id` gravável pelo jogador sem checar propriedade do personagem-alvo) e uma terceira encontrada só agora, na reauditoria obrigatória desta correção: `campaign_profiles_dev_transition_*` (o nome REAL, pós-rename da migration 0007) nunca foi removida por nenhuma correção anterior — `campaign_profiles` continuou com CRUD completo aberto a `anon` até agora. Ver `## 0-7` para a auditoria completa, a decisão de modelo canônico de propriedade de personagem, e a lista das 7 RPCs novas.
- **Validação integrada** (migrations `0033`/`0034`, esta sessão): aplica as 8 migrations pendentes (0025-0032) contra o Supabase real do projeto, roda a matriz completa de RLS/RPC com identidades reais, reproduz os 2 ataques críticos pedidos, e corrige 2 falhas concretas encontradas só nesta rodada. Ver `## 0-8` para o registro completo.
- **Rodada de concorrência simultânea real + aceite de browser** (esta sessão): a rodada anterior (`## 0-8`) só tinha comprovado concorrência SEQUENCIAL (uma chamada depois da outra via MCP) e não tinha conseguido abrir o Browser pane. Esta rodada retificou o status para deixar essa distinção explícita, construiu um script com clientes `@supabase/supabase-js` independentes e sessões reais disparando via `Promise.allSettled` para provar concorrência SIMULTÂNEA de verdade (7 cenários, todos aprovados), diagnosticou e corrigiu o servidor local (processo anterior corrompido por build+dev concorrentes na mesma `.next`), executou o aceite de browser completo (narrador, Player A, Player B, membro removido, roster do `/join`) — e encontrou e corrigiu um bug real de `revalidatePath` chamado durante render, que quebrava o aceite de convite. Ver `## 0-9` para o registro completo.
...
## 0-9. Concorrência simultânea real + aceite de browser (esta sessão)

### 0-9.1 Retificação do status documental anterior

A rodada `## 0-8` tinha declarado "Implementação concluída — aceite de browser pendente", mas o próprio texto de `## 0-8.7`/`0-8.10` já admitia duas lacunas honestas: (1) a "concorrência" comprovada ali era SEQUENCIAL (chamadas uma depois da outra via MCP, nunca duas requisições simultâneas de verdade); (2) o Browser pane nunca chegou a carregar uma página (timeout de 300s), sem diagnóstico de processo/porta/HTTP que distinguisse "ambiente indisponível" de "servidor quebrado". O status foi corrigido no início desta rodada, ANTES de rodar qualquer teste novo, para **"Implementação parcial — validação concorrente e aceite de browser pendentes."** — sem regredir para a redação genérica anterior ("validação transacional e de RLS em Supabase pendente"), já que RLS e RPC seguem genuinamente comprovadas contra o banco real (`## 0-8.4`/`0-8.5`/`0-8.6`).

### 0-9.2 Método: por que a rodada anterior não provava simultaneidade, e o que muda aqui

`SET ROLE`/`request.jwt.claim.sub` via MCP compartilha a MESMA conexão/sessão de banco entre "identidades" alternadas sequencialmente — mesmo quando duas chamadas são emitidas em blocos de tool-call próximos, não há garantia (nem evidência) de que as transações se sobrepõem de fato. Para provar concorrência real, esta rodada usou `scripts/dev/validate-campaign-session-concurrency.mjs` (novo arquivo): dois ou mais clientes `@supabase/supabase-js` distintos, cada um com sua própria sessão real (`signInWithPassword` com usuário/senha reais, criados via `admin.auth.admin.createUser` só para preparação de fixture), disparando as chamadas concorrentes via `Promise.allSettled` — sem `await` sequencial entre elas. O papel `service_role` foi usado EXCLUSIVAMENTE para criar/inspecionar/limpar fixtures e simular corrupção administrativa; nunca como identidade sob teste.

### 0-9.3 Cenários executados e resultados (todos contra o Supabase real do projeto)

1. **Reivindicação simultânea** — Player A e Player B disparam `claim_campaign_profile` no MESMO perfil ao mesmo tempo. Resultado: exatamente um venceu (o outro recebeu erro de violação de unicidade), `user_id` final único e consistente, exatamente 1 linha de perfil — **aprovado**.
2. **Dupla entrada simultânea** — o mesmo dono de perfil, duas sessões de login independentes, ambas chamando `enter_campaign_profile` ao mesmo tempo no mesmo perfil. Resultado: exatamente 1 sessão terminou `active` — **aprovado**.
3. **Heartbeat vs. saída simultâneos** — `heartbeat_profile_session` e `leave_campaign_profile` disparados ao mesmo tempo para a mesma sessão (duas conexões independentes). Resultado: status final da sessão terminal e consistente (`exited`), perfil corretamente desbloqueado — **aprovado**.
4. **`force_release` vs. heartbeat simultâneos** — dono chama `force_release_campaign_profile` enquanto o jogador chama `heartbeat_profile_session`, ao mesmo tempo. Resultado: perfil terminou desbloqueado E o token do jogador deixou de autorizar `get_character_for_profile_session` depois, independente de qual operação "venceu" a corrida — **aprovado**.
5. **Save vs. troca de personagem ativo simultâneos** — jogador chama `save_character_for_profile_session` no personagem A1 enquanto o dono chama `set_campaign_profile_active_character` trocando para A2, ao mesmo tempo. Resultado: A2 nunca foi tocado; A1 terminou em estado consistente (nunca corrompido) — **aprovado**. **Variante hostil (5b)**: dono tenta trocar o personagem ativo para um personagem que pertence a OUTRO perfil — rejeitado corretamente pela RPC (mesma checagem de propriedade real já comprovada em `## 0-8.6`); um `save` hostil concorrente mirando esse personagem alheio não o tocou — **aprovado**.
6. **Expirar vs. heartbeat simultâneos** — uma sessão é administrativamente tornada obsoleta (`last_seen_at` retroativo, simulando inatividade real) enquanto outra permanece fresca; `expire_stale_profile_sessions` e ambos os `heartbeat_profile_session` disparados ao mesmo tempo. Resultado: a sessão obsoleta terminou expirada/bloqueada; a sessão fresca permaneceu `active` — **aprovado**.

**7 cenários aprovados, 0 reprovados.** Em execuções diferentes, QUAL operação "venceu" cada corrida variou de fato (evidência de que a simultaneidade era real, não determinística por ordem de chamada) — mas os invariantes exigidos se mantiveram em todas as execuções, o que é a prova de proteção transacional correta sob concorrência genuína, não só de um resultado sequencial determinístico.

### 0-9.4 Diagnóstico e correção do servidor local

Antes de abrir o Browser pane, o processo já ocupando a porta 3000 foi inspecionado (PID, `cwd`, tempo de execução) e confirmado pertencente a este projeto, mas apresentava sintomas de travamento (conexões TCP aceitas mas sem resposta, ou resposta vazia) — causa raiz identificada: uma sessão anterior tinha rodado `npm run build` (build de produção) enquanto esse MESMO processo `next dev` já estava ativo, e ambos compartilham o diretório `.next`, corrompendo o estado do servidor de desenvolvimento em execução. Nenhuma ação destrutiva manual foi necessária — o processo antigo terminou sozinho durante o diagnóstico. Um servidor NOVO foi então iniciado corretamente via a ferramenta de preview (que usa `npm run dev`, o mesmo script de `package.json`, na porta 3000), e confirmado saudável por `curl` real (não só a rota raiz) antes de qualquer navegação: `/login` → `HTTP 200`; `/` → `HTTP 404` (esperado — não existe rota raiz real neste projeto, não é sinal de defeito); logs do servidor sem erros.

### 0-9.5 Aceite de browser — narrador

Login real do owner de fixture → dashboard (`/mesas`, sem mesas listadas para outros usuários, isolado corretamente) → abriu a mesa de fixture → visualizou perfis → vinculou personagem a perfil via dropdown "vincular" (`set_campaign_profile_active_character`, RPC confirmada funcionando ao vivo) → testou Encerrar Rodada e Encerrar Cena (sem erro) → abriu a biblioteca de conteúdo da campanha (sem erro, sem vazamento) → logout. Nenhum erro 500, nenhum erro de console, todas as requisições de rede 200.

### 0-9.6 Aceite de browser — Player A

Abriu o link de convite real de fixture → login → convite aceito (após a correção do bug de `revalidatePath`, ver `## 0-9.8`) → tela "Escolha seu perfil" mostrou só os perfis NÃO reivindicados → reivindicou o perfil A → entrou na sessão do perfil → abriu a ficha → editou um campo → salvou (`save_character_for_profile_session`) → recarregou a página → confirmou que a edição persistiu → confirmou que o perfil B não aparecia na própria lista de perfis (isolamento). Nenhum erro de console, todas as requisições 200.

### 0-9.7 Aceite de browser — Player B e verificação de isolamento

Login real do Player B de fixture → aceitou o mesmo convite → tela de reivindicação mostrou SOMENTE "Perfil B (browser)" (perfil A, já reivindicado por outro jogador, corretamente ausente — confirma o comportamento do roster, ver `## 0-9.9`) → reivindicou o perfil B → entrou na sessão → abriu a própria ficha (perfil sem personagem vinculado ainda — mensagem coerente, sem erro) → **teste hostil**: navegou diretamente para a URL de ficha do perfil A (`profileId` de A, manipulado manualmente na URL, com a sessão de B) → bloqueado corretamente com "Sessão inválida ou expirada. Entre novamente pelo convite." — sem vazamento de dado do personagem/perfil de A, sem erro 500, sem erro de console; a requisição de rede correspondente retornou 200 (a página renderizou o bloqueio server-side, não um erro HTTP). **Isolamento de Player B confirmado.**

### 0-9.8 Bug real encontrado e corrigido: `revalidatePath` chamado durante render

Ao tentar aceitar o convite pela primeira vez como Player A, o browser mostrou a página de erro "Não foi possível entrar nesta mesa", com o erro real do Next.js dev: `Route /join/[token] used "revalidatePath /mesas/<id>" during render which is unsupported`. Causa raiz: `acceptCampaignInvite` (`src/lib/campaignContent/campaignContentServerActions.ts`) é chamada diretamente do corpo de render do Server Component `src/app/join/[token]/page.tsx` — não de uma Server Action real disparada por interação do usuário — e o Next.js proíbe `revalidatePath` nesse contexto. **Correção aplicada**: removida a chamada a `revalidatePath` dentro de `acceptCampaignInvite` (substituída por um comentário explicando por quê). Verificado seguro: `/mesas/[campaignId]/page.tsx` já tem `export const dynamic = "force-dynamic"`, então a rota nunca é cacheada estaticamente — a chamada removida já era redundante mesmo quando "funcionava". `npx tsc --noEmit` confirma sem erros depois da mudança; o fluxo completo de Player A foi repetido do zero depois da correção e funcionou de ponta a ponta. Este é o ÚNICO bug de código novo encontrado nesta rodada — corrigido minimamente, sem refactor preventivo, sem tocar nenhuma migration aplicada.

### 0-9.9 Membro removido (fixture administrativa) e roster do `/join`

**Membro removido**: um quarto usuário de fixture reivindicou um perfil próprio, recebeu uma sessão real (`enter_campaign_profile`, token real emitido), e SÓ DEPOIS teve `campaign_members.status` alterado para `'removed'` via operação administrativa direta (não existe UI de remoção). Recarregando a ficha com a MESMA sessão de browser (previamente válida): bloqueado corretamente com "Sessão inválida ou expirada. Entre novamente pelo convite." — sem loop infinito, sem erro 500, sem erro de console. Confirma em browser real, e não só por simulação SQL, o comportamento corrigido em `## 0-8.3`/migration `0034`.

**Roster do `/join`** (bloco "Perfis (N)"): inspecionado concretamente em três momentos (Player A pós-reivindicação, Player B pós-reivindicação, membro removido pós-reivindicação) — em todos, o texto mostrou exatamente "Perfis (1)" e exatamente 1 item foi renderizado abaixo, sempre o PRÓPRIO perfil do usuário logado, nunca os perfis de outros jogadores da mesma campanha. Contagem e itens exibidos batem exatamente; o texto não promete mostrar o roster completo da mesa (não há palavra como "todos" ou "da mesa inteira" no cabeçalho); nenhum campo de `user_id`/`active_character_id`/`session_token_hash`/metadado administrativo vazou na tela. **Nenhum bug encontrado neste bloco** — o comportamento observado é exatamente o esperado pela restrição de SELECT já aplicada nas correções 6/7 (jogador lê só o próprio perfil): não foi necessário ajustar texto/contagem nem criar uma RPC de roster público, porque não há divergência entre o que o texto promete e o que é mostrado.

### 0-9.10 Console e rede (todas as telas testadas)

Nenhum token bruto, senha ou chave apareceu em texto de console em nenhuma tela; nenhuma resposta 500; nenhum loop de chamadas; nenhuma chamada legítima bloqueada por RLS inesperadamente; os bloqueios hostis (perfil alheio, sessão removida) sempre retornaram uma tela coerente, nunca uma tela quebrada ou um erro não tratado.

### 0-9.11 Limpeza de fixtures

Removidos ao final, confirmado por contagem: `profile_sessions`, `characters`, `campaign_profiles` (3, incluindo o perfil extra criado para o teste de membro removido), `campaign_invites`, `campaign_members`, 1 `campaigns` de fixture, e os 4 usuários de fixture em `auth.users` (owner, Player A, Player B, removido) — `select count(*) from campaigns where id=<fixture>` e `select count(*) from auth.users where email like '%<tag-desta-rodada>%'` retornaram `0` depois da limpeza. Arquivos de script/credenciais usados só para o aceite de browser (fora do repositório, na scratchpad da sessão) também foram apagados ao final.

### 0-9.12 Validação técnica local desta rodada

`git status --short` limpo ao final (só as mudanças intencionais: 2 docs, o fix de `revalidatePath`, o novo script de concorrência); `git diff --check` sem problemas de espaço em branco; `next-env.d.ts` verificado sem diff residual depois de `tsc`/`build` (revertido quando alterado automaticamente); `npx tsc --noEmit` sem erros; `npm run build` sucesso (servidor de preview parado antes do build para não repetir a corrupção descrita em `## 0-9.4`); harness da Etapa 12 (`validate-campaign-homebrew.mjs`) **18/18**, inalterado; validador estruturado (`validate-profile-session-lockdown.mjs`) **78/78**, inalterado; script de concorrência simultânea (`validate-campaign-session-concurrency.mjs`) **7/7 cenários**, fixtures limpas ao final confirmado pelo próprio script.

### 0-9.13 Limitações que permanecem

- Nenhuma. Concorrência simultânea real foi comprovada para os 6 cenários pedidos; o servidor local foi diagnosticado corretamente e o aceite de browser foi executado por completo (narrador, Player A, Player B, membro removido, roster); nenhuma falha crítica ou média não corrigida permanece; RLS/RPC seguem comprovadas contra o banco real (`## 0-8`); fixtures completamente removidas. Os débitos não bloqueantes já registrados em `## 0-8` (duplicação de helpers, `search_path` ausente em `enforce_campaign_content_limits`, ausência de UI para `status='removed'`, RPCs de etapas anteriores com o mesmo sintoma do achado `0-8.2`) continuam não bloqueantes e não foram reabertos nesta rodada.

## 0-8. Validação integrada contra Supabase real (migrations 0033/0034)

### 0-8.1 Ambiente e migrations aplicadas

Projeto Supabase conectado: `ruptura-vtt` (`yvxoijexyhjjipjktfuu`, região `sa-east-1`, Postgres 17.6, `ACTIVE_HEALTHY`). `list_migrations` mostrava só 4 entradas registradas antes desta validação (`realtime_publication`, `0022_content_publishing`, `0023_editor_metadata_separation`, `import_export_book_links`) — **nenhuma das migrations 0025-0032 da Etapa 12 tinha sido aplicada neste projeto até agora**, confirmado por `list_tables` (não existiam `campaign_content_documents`/`campaign_members`/etc.). Aplicadas nesta sessão, em ordem, via `apply_migration`: `0025`, `0026`, `0027`, `0028`, `0029`, `0030`, `0031`, `0032`, e as duas novas desta validação, `0033` e `0034` — todas com `{"success":true}`, confirmadas depois em `list_migrations`.

### 0-8.2 Achado novo 1 — `anon` conseguia executar toda RPC "authenticated-only" (migration 0033)

`get_advisors(type=security)` + consulta direta a `has_function_privilege('anon', <oid>, 'EXECUTE')` confirmaram que **todas** as RPCs da Etapa 12 — inclusive as que todo o histórico de migrations descreve como "só `authenticated`" (`is_campaign_owner`, `claim_campaign_profile`, `set_campaign_profile_active_character`, `force_release_campaign_profile`, `publish_campaign_content_draft`, etc.) — eram executáveis por `anon`. Causa raiz identificada via `pg_default_acl`: este projeto tem uma default privilege de schema (`alter default privileges ... grant execute on functions to anon, authenticated, service_role`, aplicada pelo dono `postgres`) que concede EXECUTE a `anon` em toda função NOVA no momento da criação — uma entrada de ACL própria para `anon`, separada do pseudo-papel `PUBLIC`. O padrão usado em TODAS as migrations da Etapa 12 (`revoke all on function X from public; grant execute on function X to authenticated;`) nunca revogava essa entrada específica de `anon` — só revogava o privilégio implícito de `PUBLIC`.

Impacto avaliado função a função: a maioria já falhava com segurança porque valida `auth.uid() is null` internamente (nenhum bypass de autorização ativo confirmado). A exceção real: os helpers `can_read_character`/`can_manage_character`/`can_access_campaign_profile`/etc. aceitam `check_user_id` como parâmetro explícito — um chamador `anon` podia invocar a RPC com um `check_user_id` arbitrário e obter um boolean revelando pertencimento (oráculo de baixo risco, mas informação real vazada sem autenticação).

Corrigido pela migration `0033`: `revoke execute on function X from anon` explícito para as 19 RPCs/helpers da Etapa 12 que devem ser só `authenticated`. As 7 RPCs de sessão de jogador (`enter_campaign_profile`, `heartbeat_profile_session`, `leave_campaign_profile`, `validate_profile_session_token`, `expire_stale_profile_sessions`, `get_character_for_profile_session`, `save_character_for_profile_session`) permanecem chamáveis por `anon` — por design (Opção B, perfil não reivindicado). Confirmado depois via a mesma consulta `has_function_privilege`: `anon_can_execute = false` para as 19; `true` para as 7. **Escopo**: só as RPCs da Etapa 12 (0025-0032). RPCs de etapas anteriores com o mesmo sintoma (`publish_content_draft`, `import_content_drafts`, `archive_content_document`, `is_content_admin`) NÃO foram tocadas — fora do escopo desta validação (não reabrir o editor de conteúdo oficial); registradas como achado correlato, não corrigidas.

### 0-8.3 Achado novo 2 — membro `removed` mantinha sessão de personagem válida (migration 0034)

Reproduzido explicitamente: um usuário com perfil reivindicado e uma sessão já emitida (`enter_campaign_profile`, token real) continuava lendo e (por extensão da mesma checagem) tentando escrever o próprio personagem NORMALMENTE mesmo depois de `campaign_members.status` virar `'removed'` — `get_character_for_profile_session`/`save_character_for_profile_session`/`heartbeat_profile_session`/`enter_campaign_profile` só verificavam `campaign_profiles.user_id = auth.uid()`, nunca `campaign_members.status`. A remoção de um membro não revogava sessões de personagem já emitidas.

Corrigido pela migration `0034`: as 4 RPCs passam a exigir também `is_campaign_member(campaign_id, user_id)` (que já checa `status='active'`) quando o perfil já foi reivindicado (`user_id is not null`) — perfis não reivindicados continuam por token puro, sem checagem de membership (não têm usuário para checar). Reproduzido de novo depois da correção: `get_character_for_profile_session` devolve vazio e `heartbeat_profile_session` lança exceção explícita para o mesmo usuário/token que antes funcionava.

### 0-8.4 Schema efetivo confirmado (consultas diretas, não migration source)

`pg_policies` para `campaign_profiles`/`profile_sessions`/`characters`: nenhuma policy `anon` ou `dev_transition`/`dev_anon` remanescente em nenhuma das três tabelas; `campaign_profiles` tem exatamente 4 policies (`authenticated`, select/insert/update/delete, UPDATE owner-only); `characters` tem exatamente 4 (`authenticated`); `profile_sessions` tem exatamente 1 (`profile_sessions_owner_all`, owner-only, `ALL`). `information_schema.role_table_grants`: zero linhas para `grantee='anon'` nas três tabelas — confirmado por tentativa direta também (`SET ROLE anon` + SELECT/INSERT/UPDATE/DELETE nas três tabelas → `permission denied for table X` em todas as 12 combinações, um erro de GRANT, mais forte que uma simples RLS vazia).

### 0-8.5 Matriz de identidades (fixtures reais, `SET ROLE authenticated` + `request.jwt.claim.sub`, nunca service role como prova)

Fixtures criadas: 6 usuários reais em `auth.users` (owner, Player A, Player B, outsider, invited, removed — emails `validation-etapa12-*@ruptura.dev`), 1 campanha, 1 convite real (hash SHA-256 de um token conhecido só do teste), 3 perfis, 2+1 personagens, 1 rascunho de homebrew publicado.

- **Owner**: `accept_campaign_invite` n/a (é o dono); criou 3 perfis via INSERT direto (policy `authenticated_insert` owner-only); vê todos os perfis/personagens da campanha; `set_campaign_profile_active_character` funciona para vínculo correto e REJEITA vínculo incorreto (ver ataque 2); publicou homebrew via `publish_campaign_content_draft`.
- **Player A / Player B**: aceitaram convite via `accept_campaign_invite` (RPC real, não fixture direta) → `campaign_members.status='active'`; `list_claimable_campaign_profiles` mostrou os 3 perfis livres para ambos ANTES de reivindicar; A reivindicou o perfil A, B reivindicou o B via `claim_campaign_profile` (RPC real); B tentando reivindicar o perfil de A → rejeitado (`unique_violation`, "Este perfil já pertence a outro jogador"); SELECT em `campaign_profiles`/`characters` retorna SOMENTE a própria linha para cada um (isolamento recíproco confirmado); UPDATE direto em `campaign_profiles`/`characters` (próprios ou alheios) → 0 linhas afetadas (bloqueado por RLS); `enter_campaign_profile`/`heartbeat_profile_session` funcionam para a própria sessão; reentrada gera token novo e invalida o antigo (heartbeat com token velho → rejeitado, sessão não mais `active`).
- **Invited** (`campaign_members.status='invited'`, nunca aceitou de fato): `list_claimable_campaign_profiles` vazio; SELECT em `campaign_profiles` vazio (não é owner nem reivindicou nada).
- **Removed** (fixture administrativa: reivindicou perfil como `active`, RECEBEU um token de sessão real via `enter_campaign_profile`, DEPOIS `status` virou `'removed'`): `list_claimable_campaign_profiles` vazio; `get_character_for_profile_session` com o token JÁ EMITIDO — achado 2 acima, corrigido pela migration `0034` (antes da correção: retornava o personagem; depois: vazio); `heartbeat_profile_session` com o mesmo token — antes: sucesso; depois da correção: exceção explícita.
- **Outsider** (autenticado, nunca convidado): SELECT em `campaign_profiles`/`characters` vazio; `list_claimable_campaign_profiles` vazio.
- **Anon**: 12/12 operações diretas bloqueadas (SELECT/INSERT/UPDATE/DELETE × `campaign_profiles`/`profile_sessions`/`characters`, todas `permission denied`); `list_claimable_campaign_profiles`/`force_release_campaign_profile` → `permission denied for function` (confirma a correção `0033`).

### 0-8.6 Ataques críticos reproduzidos

- **Ataque 1 (forja de token)**: Player A tentou (a) `INSERT` em `profile_sessions` com hash de token escolhido pelo atacante → bloqueado por RLS (`new row violates row-level security policy`); (b) `UPDATE` direto no `session_token_hash` da PRÓPRIA sessão real → 0 linhas afetadas (só owner tem UPDATE direto); (c) chamar `get_character_for_profile_session` com IDs reais mas o token escolhido pelo atacante → vazio (hard check rejeita); (d) controle positivo com o token REAL → devolveu o personagem corretamente. **Bloqueado em toda camada.**
- **Ataque 2 (redirecionamento de personagem)**: Player A tentou (a) `UPDATE` direto `active_character_id` do próprio perfil para o personagem de B → 0 linhas afetadas; (b) chamar `set_campaign_profile_active_character` (não é owner) → exceção "Sem acesso de narrador"; (c) MESMO O OWNER tentando vincular o personagem de B ao perfil de A → exceção "Este personagem não está vinculado a este perfil" (a RPC não confia em quem está chamando, valida o vínculo real); (d) corrupção administrativa simulada (UPDATE privilegiado direto, fora da RPC, para reproduzir dado legado inconsistente) apontando `active_character_id` do perfil A para o personagem de B → **mesmo assim**, `get_character_for_profile_session` (token real de A) devolveu vazio e `save_character_for_profile_session` não alterou nada (0 linhas), ambos pela checagem `characters.profile_id = p_profile_id`. Personagem de B confirmado intocado depois. Dado corrompido restaurado imediatamente. **Bloqueado em toda camada, inclusive contra dado corrompido administrativamente.**

### 0-8.7 Concorrência

- Reivindicação dupla: B tentando reivindicar o perfil já reivindicado por A → rejeitado de forma controlada (`unique_violation`), confirmando o `for update` + índice único parcial funcionando.
- Rotação de sessão: reentrada no mesmo perfil gera uma sessão nova e invalida a anterior (`heartbeat` com o token antigo → rejeitado, status não mais `active`).
- **Limitação honesta**: verdadeira concorrência SIMULTÂNEA (duas requisições no mesmíssimo instante) não é reproduzível por esta interface (chamadas sequenciais via MCP, não paralelas de verdade) — o que foi comprovado é o mecanismo de proteção (`for update`, invalidação na entrada) e o resultado determinístico de tentativas sequenciais, não uma corrida real de threads. Documentado como limitação, não escondido.

### 0-8.8 Fluxo de conteúdo (smoke test, sem regressão)

Owner criou rascunho de homebrew (`campaign_content_drafts`) e publicou via `publish_campaign_content_draft` (RPC real). Player A (membro ativo) leu o conteúdo publicado (`campaign_content_documents`, status `published`) corretamente; Player A NÃO conseguiu ler o rascunho (`campaign_content_drafts`, vazio — owner-only); outsider NÃO conseguiu ler o conteúdo publicado (vazio — exige membership). Nenhuma regressão encontrada. Referências/comparação de três vias/limites NÃO foram reabertos (nenhuma lacuna nova alegada ou encontrada) — cobertura permanece "completa no escopo representável pelo schema público e pelos consumidores atuais".

### 0-8.9 Limpeza de fixtures

Todas as linhas de teste removidas ao final: `campaign_content_changelog`, `campaign_content_editor_metadata`, `campaign_content_references`, `campaign_content_documents`, `campaign_content_drafts`, `table_logs`, `profile_sessions`, `characters`, `campaign_profiles`, `campaign_invites`, `campaign_members`, `campaigns`, e os 6 usuários fixture em `auth.users`. Confirmado por contagem final: zero linhas remanescentes em todas as tabelas tocadas.

### 0-8.10 Browser

**Não executado** — `navigate` no Browser pane excedeu 300s sem carregar a página (aba ficou sem resposta); tentativas subsequentes (`screenshot`) confirmaram que nenhuma página chegou a abrir. Um servidor de desenvolvimento já estava rodando na porta 3000 (respeitado — não foi iniciada uma segunda instância), mas o Browser pane em si não respondeu. Limitação objetiva do ambiente desta sessão, não um bloqueio do código. Nenhum resultado de browser foi inventado.

### 0-8.11 Validação técnica local

`git status --short`/`git diff --check` limpos (só `0033_*.sql`/`0034_*.sql` novos, mais a extensão do validador focado); `npx tsc --noEmit` sem erros; `npm run build` sucesso (`next-env.d.ts` sem diff); harness da Etapa 12 (`validate-campaign-homebrew.mjs`) 18/18 (inalterado); validador estruturado desta correção (`validate-profile-session-lockdown.mjs`), estendido para cobrir 0033/0034, agora **78/78** verificações estruturais.
...
- `npm run build` (Next.js/Turbopack) — sucesso; `next-env.d.ts` verificado sem diff após o build.
- `scripts/dev/validate-campaign-homebrew.mjs` — **18/18 verificações** (inalteradas).
- `scripts/dev/validate-profile-session-lockdown.mjs` — **78/78 verificações estruturais** (48 da correção 7, mais 30 novas cobrindo as migrations `0033`/`0034` desta validação).
- **Migrations aplicadas contra Supabase real**: `0025` a `0034`, todas `{"success":true}`, confirmadas em `list_migrations`. Ver `## 0-8` para o registro completo.
- **Matriz de RLS/RPC executada contra Supabase real** com identidades reais (owner, Player A, Player B, invited, removed, outsider, anon) via `SET ROLE`/`request.jwt.claim.sub` — nunca service role como prova. Ver `## 0-8.5`.
- **Os DOIS ataques críticos pedidos foram reproduzidos e bloqueados** contra o banco real — forja de token (`## 0-8.6`) e redirecionamento de personagem, inclusive contra dado corrompido administrativamente (`## 0-8.6`).
- **Duas falhas concretas adicionais encontradas e corrigidas nesta validação** (migrations `0033`/`0034`) — ver `## 0-8.2`/`0-8.3`.
- Fluxo de conteúdo (homebrew → publicação → leitura efetiva por membro → bloqueio de rascunho/outsider) testado como smoke test, sem regressão (`## 0-8.8`).
- Fixtures de teste completamente removidas ao final, confirmado por contagem (`## 0-8.9`).
- **Concorrência SIMULTÂNEA real** (clientes independentes, sessões reais, `Promise.allSettled`) — 6 cenários pedidos + 1 variante hostil, **7/7 aprovados**. Ver `## 0-9.3`.
- **Servidor local diagnosticado e corrigido** (processo corrompido por build+dev concorrentes identificado e substituído por uma instância saudável, confirmada por HTTP real). Ver `## 0-9.4`.
- **Aceite de browser completo**: narrador (`## 0-9.5`), Player A (`## 0-9.6`), Player B com teste hostil de isolamento (`## 0-9.7`), membro removido via fixture administrativa (`## 0-9.9`), roster do `/join` sem regressão (`## 0-9.9`). Console e rede limpos em todas as telas (`## 0-9.10`).
- **Bug real encontrado e corrigido nesta rodada**: `revalidatePath` chamado durante render em `acceptCampaignInvite`, quebrando o aceite de convite. Ver `## 0-9.8`.
- Fixtures da rodada de browser/concorrência completamente removidas ao final, confirmado por contagem (`## 0-9.11`).

## Verificações não executadas

- **Diagnóstico de dados legados** (`## 0-7.4`) antes de aplicar `0032` em produção — não era aplicável a este banco (schema recém-criado nesta mesma sessão pelas migrations 0025-0032; sem dados de personagem pré-existentes ligados ao fluxo de perfil reivindicado antes desta validação).

## Limitações reais (gaps honestos que permanecem)

- **Roster do `/join` não tratado como bug** (`## 0-9.9`) — `JoinClient` continua recebendo `perfisIniciais` limitado a 0-1 linha para jogador (efeito da correção 6); comportamento inspecionado nesta rodada e confirmado coerente (texto e contagem batem com o escopo real exibido, sem promessa de roster completo) — não é um achado pendente, é uma verificação concluída sem regressão.
...
- **Achado correlato não corrigido, fora de escopo**: RPCs de etapas anteriores ao 12 (`publish_content_draft`, `import_content_drafts`, `archive_content_document`, `is_content_admin`) têm o MESMO sintoma do achado `0-8.2` (executáveis por `anon` apesar de `revoke ... from public`) — não tocadas nesta validação (não reabrir o editor de conteúdo oficial). Registrado para uma futura correção dedicada.
- **Referências**: cobertura confirmada completa no escopo real do schema atual — nenhuma família adicional real foi encontrada. Não reaberto nesta validação.
...
TypeScript e build passam; 18 verificações focadas da Etapa 12 passam; 78 verificações estruturais do validador de sessão/personagem passam; migrations 0025-0034 aplicadas e confirmadas contra Supabase real; matriz de RLS/RPC com 7 identidades passou; os 2 ataques críticos pedidos foram bloqueados, inclusive contra dado corrompido administrativamente; 2 falhas concretas adicionais foram encontradas e corrigidas na primeira rodada de validação (migrations 0033/0034); **concorrência SIMULTÂNEA real comprovada nesta segunda rodada** (clientes independentes, sessões reais, `Promise.allSettled`, 7/7 cenários aprovados); **servidor local diagnosticado e corrigido**; **aceite de browser completo executado** (narrador, Player A, Player B com teste hostil, membro removido, roster do `/join`), sem erro de console ou de rede em nenhuma tela; **1 bug real adicional encontrado e corrigido nesta rodada** (`revalidatePath` chamado durante render, quebrando o aceite de convite); fluxo de conteúdo sem regressão; todas as fixtures de teste (das duas rodadas) completamente removidas, confirmado por contagem; nenhuma publicação automática no catálogo oficial; nenhuma escrita em `content_documents`/`content_drafts` oficiais; nenhuma tabela de marketplace criada; nenhuma instância de personagem real foi tocada ou apagada; nenhum backfill especulativo de jogador, perfil ou personagem; nenhuma regra mecânica (Aljava, MIT/PD, capacidade 15, etc.) foi alterada; nenhuma policy `anon` remanescente em `campaign_profiles`/`profile_sessions`/`characters` (confirmado no banco real, não só no texto da migration); jogador sem UPDATE direto em `campaign_profiles`; `active_character_id` só gravável por RPC que valida propriedade real do personagem, mesmo contra dado corrompido. A Etapa 11 permanece "Implementação parcial — integração editorial de drag pendente", inalterada. **Não avancei para nenhuma etapa adicional. Não declaro o Editor Universal concluído.**
