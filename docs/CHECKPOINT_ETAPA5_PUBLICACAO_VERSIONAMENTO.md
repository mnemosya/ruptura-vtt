...
>
> **Nota de correção (rodada posterior à anterior):** o achado "item publicado via Editor Universal falha a revalidação de schema oficial na importação" (`categoria_label`/`raridade_label` ausentes), registrado abaixo como "não corrigido nesta rodada", **foi corrigido** em rodada subsequente — ver `docs/CHECKPOINT_CORRECAO_SCHEMA_ITEM_ROUNDTRIP.md`. O round-trip real de item (exportar → reimportar sem edição manual do JSON) foi provado contra o Supabase real nessa rodada.
