...
## 13. Ajustes feitos durante a Etapa 10 (drones, robôs, companheiros e Trama)

1. **`EfeitoFilho`/`EfeitoEditavel` ganharam 6 novos membros**: `companheiro`, `modificar_companheiro`, `acao_companheiro` (com `efeitosConsequencia: EfeitoFilho[]` reaproveitando o catálogo universal, limitado a `MAX_EFEITOS_CONSEQUENCIA_ACAO_COMPANHEIRO=3`, nunca contendo outro `acao_companheiro` — validado em runtime, não estruturalmente, única exceção ao padrão desde a Etapa 7, justificada pelo teto de automação sempre `lembrete`), `programar_gatilho` (reaproveita o campo comum `gatilho`, nunca duplicando o conceito), `parear`, `acao_trama`. Todos representáveis só em TALENTO (`familia:"companheiro"`/`"trama"`, valores reais do enum de família — já reconhecidos por `legacyConversion.ts::FAMILIAS_INCOMPATIVEIS` para conteúdo legado). Nenhum `content_type` novo — decisão documentada no checkpoint (drone/robô não são entidades reutilizáveis hoje, sem catálogo publicado).
2. **RAM não ganhou tipo novo** — reaproveita `alterar_recurso` (`recurso:"ram"`, já suportado desde a Etapa 4/8).
3. **5 bugs reais corrigidos** (descobertos ao validar contra `schema_talentos_v1_3.json`, que é FECHADO — `additionalProperties:false`, ~90 chaves fixas): `efeito_temporario`, `modificar_instancia`, `conceder_item`, `consumir_item` e `alterar_disponibilidade` emitiam, para TALENTO, chaves que não existem nesse enum (`bonus_pericia`, `operacao`, `item_slug`, `destino`, `fornecedor`, `limite`, etc.) — nunca detectado porque os harnesses da Etapa 8/9 só validaram esses tipos contra item/rune (`additionalProperties:true`, mascarava o problema). Corrigido reaproveitando chaves reais (`acao`, `identifica`, `escopo`, `max_unidades`, `pericias`, `requisito`) — `escopo`/`identifica` exigiram `array` (nunca string solta) e `teste` exige `object|string|array` (nunca `boolean`), ambos também corrigidos nos 6 tipos novos.
4. **Nenhuma migration nova, nenhum código tocado em `talentEngine.ts`** — Droneiro/Mecatrônico/Tecelão permanecem 100% bespoke, comprovadamente não-generalizáveis (auditoria por inspeção direta de cada função, nunca por semelhança de nome).
...
## 15. Ajuste feito numa correção posterior à Etapa 11 (contrato de item)

`categoria_label`/`raridade_label` do item, exigidos pelo schema oficial de equipamentos (`content/schema_equipamentos_v1_2.json`, `required`), **nunca fazem parte do envelope editável** (`CamposEditaveis`/`CamposItem`) — o editor só expõe `categoria`/`raridade` (os valores canônicos). Os dois labels são estritamente DERIVADOS desses valores no servidor, na hora de serializar para publicação (`src/lib/contentSchema/itemLabels.ts`, único lugar que os deriva) — nunca aceitos do rascunho/client, nunca duplicados em mais de um mapa. Mesmo padrão já usado para `categoria`/`categoria_label` de runa (§12, sempre fixados como `"runa"`/`"Runa"`), generalizado aqui para um enum fechado (10 categorias, 5 raridades) em vez de um valor único. Detalhes completos, incluindo o defeito que motivou este ajuste (round-trip de item falhava contra o schema oficial), em `docs/CHECKPOINT_CORRECAO_SCHEMA_ITEM_ROUNDTRIP.md`.
