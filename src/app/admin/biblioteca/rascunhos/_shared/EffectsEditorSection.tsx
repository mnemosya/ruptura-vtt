...
export function EffectsEditorSection({
  ...
  return (
    <div>
      {ordenados.length === 0 && <p style={{ color: "#7d7d8a", fontSize: 13 }}>Nenhum efeito configurado ainda.</p>}
      ...
      {ordenados.map((efeito, indice) => {
        ...
        return (
          <div key={efeito.id} data-testid="efeito-editor-card" style={{ ...sectionStyle, opacity: efeito.habilitado ? 1 : 0.55 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
              <strong>
                {LABEL_TIPO[efeito.tipo]}
                {efeito.nomeOpcional ? ` — ${efeito.nomeOpcional}` : ""}
              </strong>
              <span style={{ color: MODO_AUTOMACAO_COR[diagnostico.modoAutomacao], fontSize: 13 }}>
                {MODO_AUTOMACAO_SIMBOLO[diagnostico.modoAutomacao]} {MODO_AUTOMACAO_LABEL[diagnostico.modoAutomacao]}
              </span>
            </div>

            <div style={{ display: "flex", gap: 8, marginBottom: 10, flexWrap: "wrap" }}>
              <button type="button" onClick={() => mover(efeito.id, -1)} disabled={indice === 0} style={buttonStyle}>
                ↑ mover para cima
              </button>
              <button type="button" onClick={() => mover(efeito.id, 1)} disabled={indice === ordenados.length - 1} style={buttonStyle}>
                ↓ mover para baixo
              </button>
              <button type="button" onClick={() => atualizar(efeito.id, { habilitado: !efeito.habilitado })} style={buttonStyle}>
                {efeito.habilitado ? "Desabilitar" : "Habilitar"}
              </button>
              <button type="button" onClick={() => duplicar(efeito.id)} style={buttonStyle}>
                Duplicar
              </button>
              <button type="button" onClick={() => remover(efeito.id)} style={{ ...buttonStyle, color: "#e08a8a" }}>
                Remover
              </button>
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 10, marginBottom: 10 }}>
              <label style={labelStyle}>
                Nome opcional
                <input value={efeito.nomeOpcional ?? ""} onChange={(e) => atualizar(efeito.id, { nomeOpcional: e.target.value || undefined })} style={inputStyle} />
              </label>
              <label style={labelStyle}>
                Gatilho *
                <select value={efeito.gatilho ?? ""} onChange={(e) => atualizar(efeito.id, { gatilho: e.target.value || undefined })} style={inputStyle}>
                  <option value="">—</option>
              ...
              <label style={labelStyle}>
                Alvo *
                <select value={efeito.alvo ?? ""} onChange={(e) => atualizar(efeito.id, { alvo: e.target.value || undefined })} style={inputStyle}>
                  <option value="">—</option>
            ...
            <label style={{ ...labelStyle, marginTop: 10 }}>
              Texto de log
              <input value={efeito.textoLog ?? ""} onChange={(e) => atualizar(efeito.id, { textoLog: e.target.value || undefined })} style={inputStyle} />
            </label>
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "flex-end" }}>
        <label style={labelStyle}>
          Adicionar efeito
          <select value={tipoParaAdicionar} onChange={(e) => setTipoParaAdicionar(e.target.value as TipoEfeitoMvp)} style={inputStyle}>
