...
import {
  Archive, ArchiveRestore, ArrowDown, ArrowUp, Check, Copy,
  MonitorPlay, MoreVertical, Pencil, SlidersHorizontal, Trash2, Users, X,
} from "lucide-react";
import type {
...
export interface PropsCartaoCena {
  ...
  /** Um JOGADOR está sendo arrastado e paira sobre este ladrilho. */
  alvoDeJogador?: boolean;
  /** Abrir os parâmetros DESTA cena (tamanho, grade, cor) na gaveta. */
  onConfigurar: () => void;
...
/**
 * Onde o menu cabe, medido contra a JANELA DO NAVEGADOR.
 *
 * Alinhado pela direita do botão — ele nasce no canto do ladrilho, e
 * alinhar pela esquerda o jogaria pra fora. Abre pra cima quando não
 * sobra altura embaixo.
 */
function posicaoDoMenu(botao: HTMLElement | null): { left: number; top: number } | null {
  if (!botao) return null;
  const b = botao.getBoundingClientRect();
  const cabeEmbaixo = window.innerHeight - b.bottom > ALTURA_MENU + 8;
  return {
    left: Math.max(8, b.right - LARGURA_MENU),
...
export function CartaoCena(p: PropsCartaoCena) {
  ...
  return (
    <li
      ...
      data-alvo-jogador={p.alvoDeJogador || undefined}
      ...
      onDragLeave={p.arrasto.onDragLeave}
    >
      {/* O MAPA é o cartão. Reconhecer uma cena é reconhecer o desenho
          dela — foi por isso que a miniatura saiu de 44px na lateral de
          uma linha para a superfície inteira do ladrilho. Sem imagem,
          a inicial do nome: distingue de relance sem prometer uma
          figura que não existe. */}
      <button
        type="button"
        className="rv-cena-palco"
        data-testid="cena-abrir"
        disabled={p.ocupada}
        onClick={p.onAbrir}
        aria-label={p.vista ? `Você já está em "${p.cena.nome}"` : `Abrir "${p.cena.nome}"`}
      >
        {p.miniaturaUrl
          ? <img src={p.miniaturaUrl} alt="" className="rv-cena-mini-img" />
          : <span className="rv-cena-inicial" aria-hidden="true">{(p.cena.nome.trim()[0] ?? "?").toUpperCase()}</span>}
        ...
        {/* Quem está NESTA cena, sobre o mapa dela: é a resposta que a
            gaveta inteira existe pra dar de relance. Contorno de acento
            em quem foi MANDADO para cá — o resto está aqui porque a
            mesa está, e os dois não podem parecer a mesma coisa. */}
        {p.jogadoresAqui.length > 0 && (
          <span className="rv-cena-presentes">
        ...
        {/* O que aparece quando um JOGADOR paira sobre o ladrilho. Sem
            isso, soltar vira aposta — a mesma razão da pasta realçar. */}
        <span className="rv-cena-solte" aria-hidden="true">Soltar aqui</span>
      </button>
      ...
      <span className="rv-cena-txt">
        {modo === "renomeando" ? (
          ...
          <span className="rv-cena-legenda">
            <span className="rv-cena-nome" data-testid="cena-nome">{p.cena.nome}</span>
            <span className="rv-cena-meta">
              ...
              <span className="rv-cena-local" data-tipo="grade">{p.cena.largura}×{p.cena.altura}</span>
            </span>
          </span>
        )}
      ...
      <span className="rv-cena-acoes">
        ...
        {!p.cena.apresentada && !arquivada && (
          ...
          >
            ...
            <span className="rv-dica rv-dica--acima">Apresentar aos jogadores</span>
          </button>
        )}
        ...
        {!arquivada && (
          <button
            ...
            aria-label={`Parâmetros de "${p.cena.nome}"`}
            data-testid="cena-configurar"
            disabled={p.ocupada} onClick={p.onConfigurar}
          >
            <SlidersHorizontal size={15} aria-hidden />
            <span className="rv-dica rv-dica--acima">Parâmetros da cena</span>
          </button>
        )}
        ...
        <span className="rv-cena-menu-casca" ref={menuRef}>
          <button
            ...
            disabled={p.ocupada}
            onClick={() => {
              // Medido AQUI, não num efeito: no efeito o menu já teria
              // renderizado uma vez sem posição — um quadro escondido
              // que pisca e que qualquer teste de "o menu abre" pega
              // em flagrante.
              if (!menuAberto) setMenuCaixa(posicaoDoMenu(menuRef.current));
              setMenuAberto((a) => !a);
            }}
          >
            <MoreVertical size={15} aria-hidden />
            {!menuAberto && <span className="rv-dica rv-dica--acima">Mais ações</span>}
          </button>
