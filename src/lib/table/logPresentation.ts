/**
 * Apresentação COMPARTILHADA de `table_logs` — rótulo, glifo, cor e
 * texto por tipo de evento, mais o dispatcher `formatTableLogEntry`.
 *
 * Estava dentro de `src/app/dev/character-sheet/components/MesaTab.tsx`,
 * um componente de rota `/dev`, mesmo já sendo consumido por produção
 * (`SessionPanel` da casca da campanha) e por script de teste. Com o
 * painel da Mesa (`vtt/_painel/ChatTab.tsx`) virando o terceiro
 * consumidor, o formatador saiu de vez para a camada de biblioteca:
 * nenhuma regra mudou, nenhum consumidor perdeu comportamento — só o
 * endereço passou a ser o certo. `MesaTab.tsx` reexporta
 * `formatTableLogEntry` para não quebrar importações antigas.
 *
 * Módulo PURO: sem `"use client"`, sem `"use server"`, sem React —
 * serve tanto a Server Component quanto a Client Component quanto a
 * script `tsx`. Nunca cai em JSON cru: tipo desconhecido passa por
 * `formatGenericLog`.
 */
...
import { TABLE_LOG_VISIBILITIES, type TableLogEntry, type TableLogVisibility } from "./types";

export const VISIBILITY_LABELS: Record<TableLogVisibility, string> = {
  public: "Pública",
  private: "Privada",
  gm: "Narrador",
};

export const VISIBILITY_FILTERS = ["todos", ...TABLE_LOG_VISIBILITIES] as const;
...
export const ENTRY_KIND_LABELS: Record<string, string> = {
  ...
  compendio_compartilhado: "Compêndio",
  /* Os três abaixo entravam no feed sem nome canônico — o card mostrava
     um rótulo e o log técnico, outro. */
  attack_damage_applied: "Dano Aplicado",
  collapse_outcome: "Colapso — Desfecho",
  combate_vtt: "Combate",
};

export function entryKindLabel(type: string): string {
...
export function entryIcon(type: string): string {
  ...
  if (type === "compendio_compartilhado") return "📖";
  return "•";
}

export function entryBorderColor(type: string): string {
  ...
  if (type === "compendio_compartilhado") return "#35c8f0";
  return "#ffb84f";
}
...
/**
 * `compendio_compartilhado` — cartão que a aba Compêndio do painel da
 * Mesa envia ao Chat ("Enviar ao Chat"). O payload guarda nome, tipo,
 * slug, origem e um resumo curto JÁ RESOLVIDOS no servidor, então o
 * cartão continua legível mesmo se o conteúdo efetivo mudar depois —
 * nunca depende de reler a Biblioteca para renderizar.
 */
function formatCompendioCompartilhado(payload: Record<string, unknown>): string {
  const nome = typeof payload.nome === "string" && payload.nome.trim() ? payload.nome : "(sem nome)";
  const categoria = typeof payload.categoriaRotulo === "string" ? payload.categoriaRotulo : "";
  const origem = typeof payload.origemRotulo === "string" ? payload.origemRotulo : "";
  const resumo = typeof payload.resumo === "string" && payload.resumo.trim() ? payload.resumo.trim() : "";
  const cabecalho = [nome, categoria].filter(Boolean).join(" — ");
  const marca = origem && origem !== "Oficial" ? ` (${origem})` : "";
  return `${cabecalho}${marca}${resumo ? `: ${resumo}` : ""}`;
}

/** Texto da mensagem de chat — aceita `text` (ficha, v0.12) ou `mensagem` (formato antigo do /dev/table). */
export function chatText(payload: Record<string, unknown>): string {
...
/**
 * Autor preferencial de uma mensagem de chat: personagem > autor
 * resolvido no servidor > perfil (legado) > "Mesa".
 *
 * `autorNome` é o campo que o Chat do painel da Mesa grava
 * (`vtt/_painel/acoes/chatPainel.ts`) — SEMPRE resolvido no servidor a
 * partir de `auth.uid()`/`characters.name`/`list_campaign_roster`,
 * nunca do que o browser mandou. Fica DEPOIS de `characterNome` só
 * porque as duas concordam quando existem juntas (o mesmo servidor
 * escreve as duas); mensagens antigas continuam caindo nos campos
 * legados e, por fim, em "Mesa".
 */
export function chatAuthor(payload: Record<string, unknown>): string {
  if (typeof payload.characterNome === "string" && payload.characterNome.trim()) return payload.characterNome;
  if (typeof payload.autorNome === "string" && payload.autorNome.trim()) return payload.autorNome;
